...
function SidebarProvider({
    ...
    const getStoredSidebarOpen = React.useCallback(() => {
    ...
    const [_open, _setOpen] = React.useState<boolean>(() => getStoredSidebarOpen());

    React.useEffect(() => {
        _setOpen(getStoredSidebarOpen());
...
function SidebarTrigger({
  ...
  return (
      <Button
          ...
          variant="ghost"
          size="icon"
          className={cn("size-7", dir === "rtl" && "[&_svg]:scale-x-[-1]", className)}
          onClick={(event) => {
            onClick?.(event);
            toggleSidebar();
          }}
  ...
  return (
      <ul
          ...
          className={cn(
              ...
              dir === "rtl"
                  ? "border-sidebar-border mr-3.5 border-r"
                  : "border-sidebar-border ml-3.5 border-l",
              "group-data-[collapsible=icon]:hidden",
              className
          )}
          {...props}
